...
// Database connection
const pool = new Pool({
  connectionString: process.env.DATABASE_URL || 'postgresql://localhost/stratgo',
...
app.get('/api/game/:id', async (req, res) => {
  try {
    ...
    res.json({
      ...
      board: game.board,
      moves: game.moves,
      turn: game.moves.length % 2 === 0 ? 'Player 1' : 'Player 2',
...
app.post('/api/game/:id/move', async (req, res) => {
  try {
    const { word, positions, player, score, capturePoints } = req.body;
    ...
    await pool.query(
      'UPDATE games SET moves = $1, total_moves = $2 WHERE id = $3',
      [JSON.stringify(moves), moves.length, req.params.id]
    );
    
    // Check if game is complete (you can add your own completion logic)
    const isGameComplete = false; // Add your game completion logic here
    
    if (isGameComplete) {
      await pool.query(
        'UPDATE games SET status = $1, completed_at = $2 WHERE id = $3',
        ['completed', new Date(), req.params.id]
      );
    }
    ...
    res.json({
      ...
      board: game.board,
      moves: moves,
      turn: moves.length % 2 === 0 ? 'Player 1' : 'Player 2',
